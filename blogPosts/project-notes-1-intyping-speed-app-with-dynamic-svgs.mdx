...
This blog does not cover every single line of code. However, you can check out the [repository](https://github.com/madeofwoods/typingtestapp) if you are intersted in going deeper.

# Setting Up The Timing Functionality
...
# Generating and Comparing Typed Characters

For generating the words to be typed, I used [fakerJS](https://fakerjs.dev/), an amazing javascript library for generating fake data.
...
I created the <Tag>calculateErrors()</Tag> function to calculate the total number of incorrect typed characters.
...
SVGs (Scalable Vector Graphics) are a really useful alternative image format which, as the name suggests, are written as vector and are scalable, redering pixel-perfect at any resolution.
Due to the way SVGs are created, there is access to a lot of data which can be manipulated dynamically to bring the SVG to life.

In order to create the speedometer, we will be taking advantage of an SVG's <Tag>stroke-dasharray</Tag> and <Tag>stroke-dashoffset</Tag> attributes.

When working with SVGs in React, you can either save them as <Tag>.svg</Tag> files or use them directly in <Tag>JSX</Tag> or <Tag>TSX</Tag> files.
